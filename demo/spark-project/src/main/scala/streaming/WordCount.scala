...
//stream.foreachRDD(rdd => rdd.foreach(x => println(x.toString() + "\n")))
		...
		val normalisedWords = tweetTextOnly.map(normaliseMessage(_))
		val noEmoticons = normalisedWords.map(replaceEmoticons(_))
		val replacedWords = noEmoticons.map(replaceUnwantedWords(_))
		
		
		replacedWords.foreachRDD(rdd => rdd.foreach(x => println(x + "\n")))
